...
if game:GetService('HttpService'):JSONDecode(loadstring(game:HttpGet('https://raw.githubusercontent.com/meandmystupidity/doggoshub/main/database.json'))).message == 'success' then
  print('Loadstring Enabled')
else
  --// trying this as it might work in some games
--// not entirely from me
...
local function try(code)
  if code:sub(1, 1) == '"' and code:sub(code:len(), code:len()) == '"' then
    local newcode = code:sub(2, code:len() -1)
    if newcode:find('"') then
      return false
    else
